...
* There is a `stats.txt` that contains complementary information, including tallies of the generations at which matrices have been produced. Each generation has three entries:
   * _sample_ which is the sample generated by the transformer (or at the start, a random sample).
   * _improved_ which is the data after some local optimisation process.
