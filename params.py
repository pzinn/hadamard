...
perms = torch.tensor(list(p for p in permutations(range(nm)) if p[0]==0 and (not fixed_sums or tuple(segment_sums[i+1] for i in p) == segment_sums[1:])), dtype=torch.long, device=device)
