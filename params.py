...
gen_decay = 0.0
sample_batch_size = 50_000  # for sampling
...
def compute_derived():
    ...
    global aut, aut_inds1, aut_inds3, perms, cst
    global rndmod, nrnd
    if n % 4 != 0:
    ...
    perms = torch.tensor(
        list(p for p in permutations(range(3)) if not fixed_sums or tuple(segment_sums[i] for i in p) == segment_sums[:3]),
        dtype=torch.long,
        device=device,
    )
...
def rotate(array0):
    array0 = array0.to(device=device).view(-1,na)
    ...
    array1=array[:,3*nn2:]
    # automorphism
    array1.index_copy_(1,aut_inds1[rnd[1]],array01)
...
def unfold(m):
    return torch.stack((mir(m[...,:nn2]),
                        mir(m[...,nn2:2*nn2]),
                        mir(m[...,2*nn2:3*nn2]),
                        m[...,3*nn2:]),dim=-2)
def score(m):
    return score_fft(fft(unfold(m)))
