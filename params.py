...
weight_decay = 0.01
max_iterations = 30
training_steps = 15_000  # for gen 0. automatically decreases with gen
...
resume = True  # True | False, whether to resume a previous run
...
def compute_derived():
    ...
    if fixed_sums:
        ...
        assert segment_sums[0]*segment_sums[0]+3*segment_sums[1]*segment_sums[1]==n
...
score_cf = torch.tensor([1,3], dtype=real_dtype, device=device).unsqueeze(1)
def fft(m):
...
def score_fft(f):  # score in terms of precomputed fft f (b, nm, nn2+1)
    return score_fft_int((score_cf.unsqueeze(2) * torch.view_as_real(f).square()).sum(dim=(1, 3)))  # sum over nm copies, over real/imag
def score(m):
