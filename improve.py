import torch
from params import na, nm, nn, nn2, device, score, score_fft, score_fft_int, fft, fixed_sums, num_ones, real_dtype, complex_dtype, eps, cst, segment_sums, verbose, config, score_cf
...
def improve_phases(arrays, scores):
    ...
    for j in range(nm):
        cnt.zero_()
        ff = score_cf * torch.view_as_real(f).square().sum(dim=-1)
        ...
        for t in range(100*na*max(1,config.num_improve)):  #?
            ...
            fmod *= cst
            new_scores = score_fft_int(ffs1[inds] + score_cf[j] * torch.view_as_real(fmod).square().sum(dim=-1))
