...
print(f"initial autocorrelation classes: {autocorrelation_classes.shape[0]} ({autocorrelation_classes.shape[0] / 2**nn:.6f} of all sequences)")
print_table_stats("autocorrelation multiplicities", autocorrelation_classes, class_multiplicities)

# now the painful bit
partial_sums = autocorrelation_classes.clone()
partial_counts = class_multiplicities.clone()
...
# 3 times in a row, create new list by adding elts of ffu to elts of lst
for i in range(3):
    if partial_sums.shape[0] < autocorrelation_classes.shape[0]:
        large_table = autocorrelation_classes
        small_table = partial_sums
        large_counts = class_multiplicities
        small_counts = partial_counts
    else:
        small_table = autocorrelation_classes
        large_table = partial_sums
        small_counts = class_multiplicities
        large_counts = partial_counts
    new_partial_sums = torch.empty((0, autocorrelation_classes.shape[1]), device=device, dtype=partial_sums.dtype)
    new_partial_counts = torch.empty((0,), device=device, dtype=torch.long)
    print(f"combination step {i+1}")
    ...
    partial_sums = canonicalise_automorphism_exact(partial_sums, symmetry_ctx)
    partial_sums, partial_counts = unique_rows_with_counts(partial_sums, partial_counts)
    print_table_stats(f"after step {i+1}", partial_sums, partial_counts)
